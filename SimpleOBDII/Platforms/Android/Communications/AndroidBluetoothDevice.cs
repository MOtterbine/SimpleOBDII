...
public partial class AndroidBluetoothDevice : IDevicesService, ICommunicationDevice
{
    ...
    public bool Open()
    {
        try
        {
            ...
            {
                listenTask = new Task(beginListen);
                listenTask.Start();
    ...
    private async void beginListen()
    {
        ...
        // if (AppShellViewModel.Instance.CommunicationChannel == null || String.IsNullOrEmpty(AppShellViewModel.Instance.CommunicationChannel.Name))
        if (String.IsNullOrEmpty(AppShellModel.Instance.CommunicationChannel))
        {
            FireErrorEvent(OS.OBDII.Constants.COMMUNICATION_DEVICE_NOT_SETUP);
            return;
        }

        // Attempt to set the device to a user-set name (which might not be available)
        if (TryDeviceConnect(AppShellModel.Instance.CommunicationChannel))
        {
        ...
        {
            FireErrorEvent($"Unable to access device: {AppShellModel.Instance.CommunicationChannel}");
        }
    ...
    private async Task Listen()
    {
        ...
        try
        {
            ...
            {
                FireDeviceEvent(new ChannelEventArgs() { Event = CommunicationEvents.Disconnected});
            }
        }
        catch (Exception ex)
        {
