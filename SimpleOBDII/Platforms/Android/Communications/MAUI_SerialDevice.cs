...
public partial class MAUI_SerialDevice : IDevicesService, ICommunicationDevice, ISerialDevice
{
    ...
    private void SetDevice(string channelName)
    {
        ...
        {
            Android.Util.Log.Error(Constants.STRING_LOG_TAG, "MAUI_SerialDevice.SetDevice({channelName}) - no channel specified ");
            return;
        }
        ...
        foreach (IDevicesService dev in this.deviceTypesList)
        {
            ...
            {
                Android.Util.Log.Info(Constants.STRING_LOG_TAG, $"MAUI_SerialDevice.SetDevice({channelName}) ");
    ...
    public bool Open()
    {
        ...
        {
            AppShellModel.Instance.LogService.AppendLog(Microsoft.Extensions.Logging.LogLevel.Error, "MAUI_SerialDevice.Open() - NO DEVICE");
            if (String.IsNullOrEmpty(this.DeviceName)) FireErrorEvent(Constants.COMMUNICATION_DEVICE_NOT_SETUP);
        ...
        AppShellModel.Instance.LogService.AppendLog(Microsoft.Extensions.Logging.LogLevel.Debug, 
                                                                        $"MAUI_SerialDevice.Open({this.CurrentDevice.DeviceName})");
        return this.CurrentDevice.Open();
    }

    public bool Close()
    {
        AppShellModel.Instance.LogService.AppendLog(Microsoft.Extensions.Logging.LogLevel.Debug,
                      $"MAUI_SerialDevice.Close({(this.CurrentDevice == null ? string.Empty : this.CurrentDevice.DeviceName)})");
    ...
    public bool Initialize() => this.CurrentDevice==null?false:this.CurrentDevice.Initialize();
